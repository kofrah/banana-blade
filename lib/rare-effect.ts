...
export function createRareEffect(scene:T.Scene,reduced:boolean){
 ...
 return {withoutEffect<T>(capture:()=>T):T{const visible=group.visible;group.visible=false;try{return capture()}finally{group.visible=visible}},trigger(x:number,y:number){age=0;group.position.set(x,y,.15);group.visible=true},update(dt:number){age+=dt;if(age>1.3){group.visible=false;return}for(let i=0;i<count;i++){positions[i*3]=velocities[i*3]*age;positions[i*3+1]=velocities[i*3+1]*age-.3*age*age;positions[i*3+2]=velocities[i*3+2]*age}geometry.attributes.position.needsUpdate=true;material.opacity=Math.max(0,1-age/1.3);ring.scale.setScalar(.15+age*(reduced?.8:2));ringMaterial.opacity=Math.max(0,.8-age*.8)},dispose(){scene.remove(group);geometry.dispose();material.dispose();ringGeometry.dispose();ringMaterial.dispose()}};
}
