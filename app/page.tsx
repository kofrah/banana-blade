'use client';
import {Camera,ArrowUpRight,Download} from 'lucide-react';
...
export default function Home(){
 ...
 const shoot=useCallback(async()=>{if(busy.current||phase!=='camera'||status!=='ready')return;const v=video.current;if(!v?.videoWidth)return;busy.current=true;const id=generation.current;setPhase('loading');setFlash(true);setCut(false);const line=resultLines[Math.floor(Math.random()*resultLines.length)];setResultLine(line);
 try{const photo=document.createElement('canvas');const scale=Math.min(1,1600/Math.max(v.videoWidth,v.videoHeight));photo.width=Math.round(v.videoWidth*scale);photo.height=Math.round(v.videoHeight*scale);const ctx=photo.getContext('2d');if(!ctx)throw new Error();ctx.drawImage(v,0,0,photo.width,photo.height);stopStream();await new Promise<void>(r=>requestAnimationFrame(()=>r()));if(id!==generation.current||!mounted.current)return;setFlash(false);setPhase('playing');
 if(host.current)cleanup.current=playSlice(host.current,photo,bladeCount,line,()=>{if(id===generation.current)setCut(true)},(image)=>{if(id===generation.current){setResult(image);setPhase('done');busy.current=false}},()=>{setPhase('camera');setStatus('error');setError('3D表示を開始できませんでした。ブラウザを更新して、もう一度お試しください。');busy.current=false});
 }catch{busy.current=false;setPhase('camera');setStatus('error');setError('撮影を完了できませんでした。もう一度お試しください。');setFlash(false)}},[phase,status,stopStream,bladeCount]);
 ...
 {phase==='done'&&<section className="result result-review" aria-label="切断結果"><div className="result-heading"><span>RESULT</span><p className="result-line">{resultLine}</p><h2>NICE SLICE.</h2></div>{result&&<img className="result-image" src={result} alt="撮影した写真を切断した結果。上下のメッセージとバナナブレイドのアイコン付き"/>}<div className="result-actions"><button className="save-result" onClick={()=>void saveResult()} disabled={saving||!result}><Download size={18}/>{saving?'保存メニューを開いています…':'結果を保存する'}</button><button className="retake-result" onClick={reset} disabled={saving}>もう一度撮る <ArrowUpRight size={16}/></button><p className="save-message" role="status">{saveMessage||'気に入ったら保存。保存せずに次の一枚へ進めます。'}</p></div></section>}
 {phase!=='done'&&<footer><div className="mode">{cameraVisible?'PHOTO / SLICE':`${bladeCount} BB / ${bladeCount} CUT${bladeCount>1?'S':''}`}</div><div className="controls"><button className="shutter" onClick={()=>void shoot()} disabled={!cameraVisible||status!=='ready'} aria-label="写真を撮影して自動切断"><span/></button><button className="blade-count" onClick={()=>setBladeCount(n=>n%3+1)} disabled={!cameraVisible} aria-label={`バナナブレイド ${bladeCount}本。タップして本数を変更`}><span aria-hidden="true">{Array.from({length:bladeCount},(_,i)=><img key={i} src="/banana-blade.png" alt="" style={{transform:`translateY(${i*6}px) rotate(-20deg)`}}/>)}</span><b aria-live="polite">{bladeCount}本</b></button></div>{cameraVisible&&<p className="hint">シャッターを押す。その先は、バナナにおまかせ。</p>}</footer>}{flash&&<div className="flash"/>}
 </main>
}
