...
export default function Home(){
 ...
 const shoot=useCallback(async()=>{if(busy.current||phase!=='camera'||status!=='ready')return;const v=video.current;if(!v?.videoWidth)return;busy.current=true;const id=generation.current;setPhase('loading');setFlash(true);setCut(false);const line=resultLines[Math.floor(Math.random()*resultLines.length)];setResultLine(line);const rare=chooseRareBlade();setIsRare(rare);
 ...
 if(host.current)cleanup.current=playSlice(host.current,photo,bladeCount,line,rare,()=>{if(id===generation.current)setCut(true)},(image)=>{if(id===generation.current){setResult(image);setPhase('done');busy.current=false}},()=>{setPhase('camera');setStatus('error');setError('3D表示を開始できませんでした。ブラウザを更新して、もう一度お試しください。');busy.current=false});
