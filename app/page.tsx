...
export default function Home(){
 ...
 if(host.current)cleanup.current=playSlice(host.current,photo,()=>{if(id===generation.current)setCut(true)},(image)=>{if(id===generation.current){setResult(image);setResultLine(resultLines[Math.floor(Math.random()*resultLines.length)]);setPhase('done');busy.current=false}},()=>{setPhase('camera');setStatus('error');setError('3D表示を開始できませんでした。ブラウザを更新して、もう一度お試しください。');busy.current=false});
 ...
 {phase!=='done'&&<footer><div className="mode">{cameraVisible?'PHOTO / SLICE':'ONE SHOT. ONE SLICE.'}</div><div className="controls"><button className="shutter" onClick={()=>void shoot()} disabled={!cameraVisible||status!=='ready'} aria-label="写真を撮影して自動切断"><span/></button></div>{cameraVisible&&<p className="hint">シャッターを押す。その先は、バナナにおまかせ。</p>}</footer>}{flash&&<div className="flash"/>}
 </main>
}
